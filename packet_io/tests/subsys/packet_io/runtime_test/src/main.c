...
PACKET_SOURCE_DEFINE(runtime_source);
PACKET_SOURCE_DEFINE(runtime_source2);

/* IMMEDIATE handler - processes synchronously (no unref needed) */
static void runtime_immediate_handler(struct packet_sink *sink, struct net_buf *buf)
{
...
static void runtime_queued_handler(struct packet_sink *sink, struct net_buf *buf)
{
	struct test_capture *capture = (struct test_capture *)sink->user_data;
...
PACKET_EVENT_QUEUE_DEFINE(runtime_queue, TEST_RUNTIME_QUEUE_SIZE);

/* Define sinks using proper macros */
PACKET_SINK_DEFINE_IMMEDIATE(runtime_immediate_sink, runtime_immediate_handler);
PACKET_SINK_DEFINE_IMMEDIATE(runtime_secondary_sink, runtime_immediate_handler);
PACKET_SINK_DEFINE_QUEUED(runtime_queued_sink, runtime_queued_handler, runtime_queue);

/* Static connections for persistent testing */
static struct packet_connection static_conn1;
static struct packet_connection static_conn2;
...
static void process_all_events(void)
{
	...
	while (packet_event_process(&runtime_queue, K_NO_WAIT) == 0) {
		count++;
...
static int send_test_packet(struct packet_source *source, uint32_t value)
{
	struct net_buf *buf;
	int ret;
...
ZTEST(packet_io_runtime, test_basic_add_remove)
{
	int ret;
	...
	/* Add connection */
	ret = packet_connection_add(&conn);
	...
	/* Remove connection */
	ret = packet_connection_remove(&conn);
...
ZTEST(packet_io_runtime, test_multiple_connections)
{
	int ret;
	struct packet_connection conn1, conn2, conn3;
	...
	/* Add all connections */
	ret = packet_connection_add(&conn1);
	zassert_equal(ret, 0, "Failed to add connection 1");

	ret = packet_connection_add(&conn2);
	zassert_equal(ret, 0, "Failed to add connection 2");

	ret = packet_connection_add(&conn3);
	...
	ret = packet_event_process(&runtime_queue, K_NO_WAIT);
	...
	ret = packet_connection_remove(&conn2);
	...
	packet_connection_remove(&conn1);
	packet_connection_remove(&conn3);
}

ZTEST(packet_io_runtime, test_duplicate_connection)
{
	int ret;
	struct packet_connection conn1, conn2;
...
ZTEST(packet_io_runtime, test_connection_persistence)
{
	int ret;
	...
	/* Add connections */
	ret = packet_connection_add(&static_conn1);
	zassert_equal(ret, 0, "Failed to add static connection 1");

	ret = packet_connection_add(&static_conn2);
	...
	/* Clean up */
	packet_connection_remove(&static_conn1);
	packet_connection_remove(&static_conn2);
}

ZTEST(packet_io_runtime, test_null_parameters)
{
	int ret;
	struct packet_connection conn;

	/* Test NULL connection */
	ret = packet_connection_add(NULL);
	zassert_equal(ret, -EINVAL, "Should reject NULL connection");

	ret = packet_connection_remove(NULL);
	zassert_equal(ret, -EINVAL, "Should reject NULL connection");
...
ZTEST(packet_io_runtime, test_already_connected)
{
	int ret;
	struct packet_connection conn;
	...
	/* Clean up */
	packet_connection_remove(&conn);
}

ZTEST(packet_io_runtime, test_remove_not_connected)
{
	int ret;
	struct packet_connection conn;
...
ZTEST(packet_io_runtime, test_double_remove)
{
	int ret;
	struct packet_connection conn;
...
#ifdef CONFIG_PACKET_IO_RUNTIME_STACK_CHECK
ZTEST(packet_io_runtime, test_stack_allocation_detection)
{
	int ret;
	struct packet_connection stack_conn;
	...
	ret = packet_connection_add(&stack_conn);

	/* In test builds, it's allowed but logged as an error */
	#ifdef CONFIG_ZTEST
	zassert_equal(ret, 0, "Stack allocation allowed in test mode");
	packet_connection_remove(&stack_conn);
	#else
	zassert_equal(ret, -EINVAL, "Stack allocation should be rejected");
	#endif
}

ZTEST(packet_io_runtime, test_heap_allocation)
{
	int ret;
	struct packet_connection *heap_conn;

	/* Allocate connection on heap */
	heap_conn = k_malloc(sizeof(struct packet_connection));
	...
	ret = packet_connection_add(heap_conn);
	zassert_equal(ret, 0, "Heap allocation should be accepted");

	/* Clean up */
	packet_connection_remove(heap_conn);
	k_free(heap_conn);
}
#endif /* CONFIG_PACKET_IO_RUNTIME_STACK_CHECK */

ZTEST(packet_io_runtime, test_cross_source_connections)
{
	int ret;
	struct packet_connection conn1, conn2, conn3;
	...
	zassert_equal(atomic_get(&runtime_capture1.count), 1, "capture1 should receive from source 2");
	zassert_equal(atomic_get(&runtime_capture2.count), 0, "capture2 should not receive");

	/* Clean up */
	packet_connection_remove(&conn1);
	packet_connection_remove(&conn2);
	packet_connection_remove(&conn3);
}
...
static void test_setup(void *fixture)
{
	...
	packet_source_reset_stats(&runtime_source);
	packet_source_reset_stats(&runtime_source2);
	packet_sink_reset_stats(&runtime_immediate_sink);
	packet_sink_reset_stats(&runtime_secondary_sink);
	packet_sink_reset_stats(&runtime_queued_sink);
#endif
}
...
ZTEST_SUITE(packet_io_runtime, NULL, NULL, test_setup, test_teardown, NULL);

#else /* CONFIG_PACKET_IO_RUNTIME_OBSERVERS */

ZTEST(packet_io_runtime, test_runtime_disabled)
{
	ztest_test_skip("Runtime observers not enabled");
}

ZTEST_SUITE(packet_io_runtime, NULL, NULL, NULL, NULL, NULL);

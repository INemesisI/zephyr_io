...
static void tcp_server_tx_handler(struct packet_sink *sink,
                                  struct net_buf *buf);

/* TCP server packet I/O interfaces */
PACKET_SOURCE_DEFINE(
    tcp_server_source); /* TCP receive - packets from network */
PACKET_SINK_DEFINE_IMMEDIATE(
    tcp_server_sink,
...
static void tcp_server_tx_handler(struct packet_sink *sink,
  ...
  LOG_INF("TCP TX: packet_id=0x%04X, %zu bytes", hdr->packet_id,
...
static void process_tcp_packet(const uint8_t *data, size_t len) {
  ...
  packet_source_send_consume(&tcp_server_source, buf, K_NO_WAIT);
}
...
K_THREAD_DEFINE(tcp_accept_tid, 1024, tcp_accept_thread, NULL, NULL, NULL, 8, 0,
                0);
K_THREAD_DEFINE(tcp_client_tid, 2048, tcp_client_thread, NULL, NULL, NULL, 8, 0,
                0);

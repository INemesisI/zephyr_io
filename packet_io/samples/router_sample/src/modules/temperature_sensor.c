...
NET_BUF_POOL_DEFINE(temp_sensor_pool, 8, sizeof(struct temp_sensor_data), 4,
                    NULL);

/* Packet source for temperature data */
PACKET_SOURCE_DEFINE(temperature_sensor_source);
...
static void simulate_sensor_reading(int16_t *temp_c, int16_t *humidity_rh) {
  ...
  current_humidity_rh =
      CLAMP(current_humidity_rh, HUMIDITY_MIN_RH, HUMIDITY_MAX_RH);
...
static void send_temperature_packet(void) {
  ...
  if (sample_counter % 10 == 1) {
    LOG_INF("Temp: %d.%02d°C, Humidity: %d.%02d%% (sample #%u)", temp_c / 100,
            abs(temp_c % 100), humidity_rh / 100, humidity_rh % 100,
            sample_counter);
  }

  /* Send packet - router will add IoTSense header with PKT_ID_SENSOR_TEMP */
  packet_source_send_consume(&temperature_sensor_source, buf, K_MSEC(100));
}
...
K_THREAD_DEFINE(temp_sensor_thread, TEMP_SENSOR_THREAD_STACK_SIZE,

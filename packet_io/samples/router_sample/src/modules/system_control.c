...
static void system_control_cmd_handler(struct packet_sink *sink, struct net_buf *buf)
{
	struct ping_cmd *cmd;
	...
	packet_source_send_consume(&system_control_source, resp_buf, K_NO_WAIT);
}

PACKET_SINK_DEFINE_IMMEDIATE(system_control_sink, system_control_cmd_handler);
PACKET_SOURCE_DEFINE(system_control_source);

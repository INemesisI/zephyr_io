...
struct router_inbound_route {
	...
	struct packet_sink *app_sink_ptr; /**< Destination sink for payload */
...
struct router_outbound_route {
	...
	struct packet_source *app_source_ptr; /**< Source being monitored */
	struct packet_sink handler_sink;      /**< Internal sink for processing */
	sys_snode_t node;                     /**< List node for route table */
};
...
#endif
	/* Network interface */
	struct packet_sink network_sink;     /**< Sink for inbound packets */
	struct packet_source network_source; /**< Source for outbound packets */
...
#define ROUTER_OUTBOUND_ROUTE_DEFINE(_router, _packet_id, _app_source)                             \
	static STRUCT_SECTION_ITERABLE(router_outbound_route,                                      \
		...
		.handler_sink = PACKET_SINK_INITIALIZER_IMMEDIATE(                                 \
			...
			&__outbound_##_router##_##_packet_id),                                     \
		.node = {NULL}};                                                                   \
	PACKET_CONNECT(&_app_source, &__outbound_##_router##_##_packet_id.handler_sink)
...
#define ROUTER_DEFINE(_name, _inbound_handler, _outbound_handler)                                  \
	struct packet_router _name = {                                                             \
		IF_ENABLED(CONFIG_PACKET_IO_NAMES, (.name = #_name, )).network_sink =              \
			PACKET_SINK_INITIALIZER_IMMEDIATE(_name##_network_sink, _inbound_handler,  \
							  &_name),                                 \

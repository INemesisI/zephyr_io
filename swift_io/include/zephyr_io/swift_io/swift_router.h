...
#ifndef ZEPHYR_INCLUDE_SWIFT_ROUTER_H_
#define ZEPHYR_INCLUDE_SWIFT_ROUTER_H_
...
struct router_inbound_route {
	...
	struct swift_io_sink *app_sink_ptr; /**< Destination sink for payload */
...
struct router_outbound_route {
	...
	struct swift_io_source *app_source_ptr; /**< Source being monitored */
	struct swift_io_sink handler_sink;      /**< Internal sink for processing */
	sys_snode_t node;                     /**< List node for route table */
};
...
#endif
	/* Network interface */
	struct swift_io_sink network_sink;     /**< Sink for inbound packets */
	struct swift_io_source network_source; /**< Source for outbound packets */
...
#define ROUTER_OUTBOUND_ROUTE_DEFINE(_router, _packet_id, _app_source)                             \
	static STRUCT_SECTION_ITERABLE(router_outbound_route,                                      \
		...
		.handler_sink = SWIFT_IO_SINK_INITIALIZER_IMMEDIATE(                                 \
			_router##_out_##_packet_id, _router_common_outbound_handler,               \
			&__outbound_##_router##_##_packet_id),                                     \
		.node = {NULL}};                                                                   \
	SWIFT_IO_CONNECT(&_app_source, &__outbound_##_router##_##_packet_id.handler_sink)
...
#define ROUTER_DEFINE(_name, _inbound_handler, _outbound_handler)                                  \
	struct packet_router _name = {                                                             \
		IF_ENABLED(CONFIG_SWIFT_IO_NAMES, (.name = #_name, )).network_sink =              \
			SWIFT_IO_SINK_INITIALIZER_IMMEDIATE(_name##_network_sink, _inbound_handler,  \
							  &_name),                                 \
		.network_source = SWIFT_IO_SOURCE_INITIALIZER(_name.network_source),                 \
...
void _router_common_outbound_handler(struct swift_io_sink *sink, struct net_buf *buf);
...
#endif /* ZEPHYR_INCLUDE_SWIFT_ROUTER_H_ */
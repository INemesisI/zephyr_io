...
static void iotsense_network_inbound_handler(struct swift_io_sink *sink, struct net_buf *buf);
...
static void iotsense_network_inbound_handler(struct swift_io_sink *sink, struct net_buf *buf)
{
	...
	int ret = swift_io_sink_deliver_consume(route->app_sink_ptr, buf, K_NO_WAIT);
	if (ret < 0) {
					      ...
					      uint16_t packet_id)
{
	...
	int ret = swift_io_source_send_consume(&router->network_source, complete_packet, K_NO_WAIT);
	if (ret < 0) {

...
static void tcp_server_tx_handler(struct swift_io_sink *sink, struct net_buf *buf);

/* TCP server packet I/O interfaces */
SWIFT_IO_SOURCE_DEFINE(tcp_server_source); /* TCP receive - packets from network */
SWIFT_IO_SINK_DEFINE_IMMEDIATE(tcp_server_sink,
...
static void process_tcp_packet(const uint8_t *data, size_t len)
{
	...
	swift_io_source_send_consume(&tcp_server_source, buf, K_NO_WAIT);
}

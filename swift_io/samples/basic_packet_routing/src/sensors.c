...
SWIFT_IO_SOURCE_DEFINE(sensor1_source);
SWIFT_IO_SOURCE_DEFINE(sensor2_source);
...
static void sensor_thread_fn(void *p1, void *p2, void *p3)
{
	...
	while (1) {
		...
		if (buf) {
			...
			ret = swift_io_source_send(&sensor1_source, buf, K_NO_WAIT);
		...
		if (buf) {
			...
			ret = swift_io_source_send_consume(&sensor2_source, buf, K_NO_WAIT);

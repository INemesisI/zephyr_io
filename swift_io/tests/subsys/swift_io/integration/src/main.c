...
LOG_MODULE_REGISTER(swift_io_integration, LOG_LEVEL_INF);
...
static void eth_filter_handler(struct swift_io_sink *sink, struct net_buf *buf)
{
...
static void eth_router_handler(struct swift_io_sink *sink, struct net_buf *buf)
{
...
static void eth_logger_handler(struct swift_io_sink *sink, struct net_buf *buf)
{
	atomic_inc(&eth_logger_capture.count);
	atomic_add(&eth_logger_capture.byte_count, buf->len);
}

static void stream_consumer1_handler(struct swift_io_sink *sink, struct net_buf *buf)
{
...
static void stream_consumer2_handler(struct swift_io_sink *sink, struct net_buf *buf)
{
...
static void stream_buffer_handler(struct swift_io_sink *sink, struct net_buf *buf)
{
...
static void bulk_processor_handler(struct swift_io_sink *sink, struct net_buf *buf)
{
...
static void bulk_storage_handler(struct swift_io_sink *sink, struct net_buf *buf)
{
...
static void perf_handler(struct swift_io_sink *sink, struct net_buf *buf)
{
...
SWIFT_IO_EVENT_QUEUE_DEFINE(eth_queue, 32);
SWIFT_IO_EVENT_QUEUE_DEFINE(stream_queue, 64);
SWIFT_IO_EVENT_QUEUE_DEFINE(bulk_queue, 32);
SWIFT_IO_EVENT_QUEUE_DEFINE(perf_queue, 128);

/* Network processing pipeline */
SWIFT_IO_SOURCE_DEFINE(eth_rx_source);
SWIFT_IO_SOURCE_DEFINE(eth_tx_source);

SWIFT_IO_SINK_DEFINE_QUEUED(packet_filter_sink, eth_filter_handler, eth_queue);
SWIFT_IO_SINK_DEFINE_QUEUED(packet_router_sink, eth_router_handler, eth_queue);
SWIFT_IO_SINK_DEFINE_IMMEDIATE(packet_logger_sink, eth_logger_handler);

/* Streaming pipeline */
SWIFT_IO_SOURCE_DEFINE(stream_producer);

SWIFT_IO_SINK_DEFINE_QUEUED(stream_consumer1, stream_consumer1_handler, stream_queue);
SWIFT_IO_SINK_DEFINE_QUEUED(stream_consumer2, stream_consumer2_handler, stream_queue);
SWIFT_IO_SINK_DEFINE_QUEUED(stream_buffer, stream_buffer_handler, stream_queue);

/* Bulk transfer pipeline */
SWIFT_IO_SOURCE_DEFINE(bulk_source);

SWIFT_IO_SINK_DEFINE_QUEUED(bulk_processor, bulk_processor_handler, bulk_queue);
SWIFT_IO_SINK_DEFINE_QUEUED(bulk_storage, bulk_storage_handler, bulk_queue);

/* Performance test sources/sinks */
SWIFT_IO_SOURCE_DEFINE(perf_source1);
SWIFT_IO_SOURCE_DEFINE(perf_source2);
SWIFT_IO_SOURCE_DEFINE(perf_source3);

SWIFT_IO_SINK_DEFINE_QUEUED(perf_sink, perf_handler, perf_queue);

/* Wire connections */
SWIFT_IO_CONNECT(&eth_rx_source, &packet_filter_sink);
SWIFT_IO_CONNECT(&eth_rx_source, &packet_logger_sink);
SWIFT_IO_CONNECT(&eth_tx_source, &packet_router_sink);
SWIFT_IO_CONNECT(&eth_tx_source, &packet_logger_sink);

SWIFT_IO_CONNECT(&stream_producer, &stream_consumer1);
SWIFT_IO_CONNECT(&stream_producer, &stream_consumer2);
SWIFT_IO_CONNECT(&stream_producer, &stream_buffer);

SWIFT_IO_CONNECT(&bulk_source, &bulk_processor);
SWIFT_IO_CONNECT(&bulk_source, &bulk_storage);

SWIFT_IO_CONNECT(&perf_source1, &perf_sink);
SWIFT_IO_CONNECT(&perf_source2, &perf_sink);
SWIFT_IO_CONNECT(&perf_source3, &perf_sink);
...
static void process_all_events(void)
{
	...
	while (swift_io_event_process(&eth_queue, K_NO_WAIT) == 0 ||
	       swift_io_event_process(&stream_queue, K_NO_WAIT) == 0 ||
	       swift_io_event_process(&bulk_queue, K_NO_WAIT) == 0 ||
	       swift_io_event_process(&perf_queue, K_NO_WAIT) == 0) {
		processed++;
		if (processed > 1000) {
			break; /* Safety limit */
		}
...
static void swift_io_integration_setup(void *fixture)
{
	reset_captures();
	process_all_events(); /* Drain any lingering events */

#ifdef CONFIG_SWIFT_IO_STATS
	swift_io_source_reset_stats(&eth_rx_source);
	swift_io_source_reset_stats(&eth_tx_source);
	swift_io_source_reset_stats(&stream_producer);
	swift_io_source_reset_stats(&bulk_source);
	swift_io_source_reset_stats(&perf_source1);
	swift_io_source_reset_stats(&perf_source2);
	swift_io_source_reset_stats(&perf_source3);

	swift_io_sink_reset_stats(&packet_filter_sink);
	swift_io_sink_reset_stats(&packet_router_sink);
	swift_io_sink_reset_stats(&packet_logger_sink);
	swift_io_sink_reset_stats(&stream_consumer1);
	swift_io_sink_reset_stats(&stream_consumer2);
	swift_io_sink_reset_stats(&stream_buffer);
	swift_io_sink_reset_stats(&bulk_processor);
	swift_io_sink_reset_stats(&bulk_storage);
	swift_io_sink_reset_stats(&perf_sink);
#endif

	ARG_UNUSED(fixture);
}

ZTEST_SUITE(swift_io_integration, NULL, NULL, swift_io_integration_setup, NULL, NULL);
...
ZTEST(swift_io_integration, test_ethernet_frame_processing)
{
	struct net_buf *buf;
	...
	ret = swift_io_source_send(&eth_rx_source, buf, K_NO_WAIT);
	net_buf_unref(buf);
...
ZTEST(swift_io_integration, test_jumbo_frame_handling)
{
	struct net_buf *buf;
	int ret;
	...
	ret = swift_io_source_send(&eth_tx_source, buf, K_NO_WAIT);
	net_buf_unref(buf);
...
ZTEST(swift_io_integration, test_network_bridge)
{
	struct net_buf *buf;
	struct ethernet_frame *frame;
	int ret;
...
ZTEST(swift_io_integration, test_audio_stream_distribution)
{
	struct net_buf *buf;
	...
	for (int i = 0; i < num_frames; i++) {
		...
		/* Create audio frame */
		frame = (struct stream_frame *)net_buf_add(buf, sizeof(*frame) + audio_frame_size);
		...
		ret = swift_io_source_send(&stream_producer, buf, K_NO_WAIT);
		net_buf_unref(buf);
...
ZTEST(swift_io_integration, test_video_frame_distribution)
{
	struct net_buf *buf;
	struct stream_frame *frame;
	int ret;
	...
	for (int chunk = 0; chunk < chunks_per_frame; chunk++) {
		...
		frame = (struct stream_frame *)net_buf_add(buf, sizeof(*frame) + chunk_size);
	...
	zassert_equal(chunks_received, chunks_per_frame, "Should receive all %d chunks",
		      chunks_per_frame);
}
...
ZTEST(swift_io_integration, test_firmware_update_distribution)
{
	struct net_buf *buf;
	...
	while (offset < total_size) {
		...
		ret = swift_io_source_send(&bulk_source, buf, K_NO_WAIT);
		net_buf_unref(buf);
		if (ret > 0) {
			chunks_sent++;
			offset += chunk_size;
	...
	TC_PRINT("Sent %d chunks, processed %d chunks, stored %d chunks (%.1f KB)\n", chunks_sent,
		 chunks_processed, chunks_stored, (double)(chunks_processed * 0.5f));
...
ZTEST(swift_io_integration, test_file_transfer_pipeline)
{
	struct net_buf *buf;
	int ret;
...
ZTEST(swift_io_integration, test_packet_fragmentation_chain)
{
	...
	/* Send chained buffer */
	ret = swift_io_source_send(&eth_rx_source, head, K_NO_WAIT);
	net_buf_unref(head);
...
ZTEST(swift_io_integration, test_sustained_throughput)
{
	struct net_buf *buf;
	int ret;
	...
	while (packets_sent < target_packets) {
		...
		ret = swift_io_source_send(&perf_source1, buf, K_NO_WAIT);
		net_buf_unref(buf);
		if (ret > 0) {
...
ZTEST(swift_io_integration, test_concurrent_high_throughput)
{
	struct net_buf *buf;
	...
	for (int i = 0; i < packets_per_source; i++) {
		...
		if (buf) {
			net_buf_add(buf, packet_size);
			ret = swift_io_source_send(&perf_source2, buf, K_NO_WAIT);
			net_buf_unref(buf);
			if (ret > 0) {
				sent[1]++;
			}
		...
		if (buf) {
			net_buf_add(buf, packet_size);
			ret = swift_io_source_send(&perf_source3, buf, K_NO_WAIT);
			net_buf_unref(buf);
			if (ret > 0) {
				sent[2]++;
			}
	...
	TC_PRINT("Total: sent %d, received %d (%.1f%% delivery)\n", total_sent, received,
		 (double)((received * 100.0f) / total_sent));
...
ZTEST(swift_io_integration, test_protocol_translation)
{
	...
	eth_frame = (struct ethernet_frame *)net_buf_add(eth_buf, sizeof(*eth_frame) + uart_len);
	...
	ret = swift_io_source_send(&eth_tx_source, eth_buf, K_NO_WAIT);
...
ZTEST(swift_io_integration, test_latency_under_load)
{
	struct net_buf *buf;
	int ret;
	...
	for (int i = 0; i < 10; i++) {
		...
		for (int j = 0; j < 5; j++) {
			...
			if (buf) {
				net_buf_add(buf, packet_size);
				swift_io_source_send(&perf_source2, buf, K_NO_WAIT);
				net_buf_unref(buf);
			}
		}
	...
	if (samples > 0) {
		...
		TC_PRINT("Latency - Avg: %u us, Min: %u us, Max: %u us\n", (avg * 1000000) / freq,
			 (min * 1000000) / freq, (max * 1000000) / freq);
	}
...
ZTEST(swift_io_integration, test_memory_pool_exhaustion)
{

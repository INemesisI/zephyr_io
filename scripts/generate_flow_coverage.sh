#!/bin/bash
# Generate coverage report from all flow tests
...
echo "Generating flow coverage report..."
echo "========================================"
...
find twister-combined-coverage -name "*.gcda" -o -name "*.gcno" | while read file; do
    ...
    testname=$(echo "$file" | sed 's|.*/\(flow\.[^/]*\)/.*|\1|')
    if [[ "$file" == *.gcda ]]; then
...
gcovr --filter=".*flow/subsys/flow/.*" --print-summary \

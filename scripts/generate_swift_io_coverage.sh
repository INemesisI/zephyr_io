#!/bin/bash
# Generate coverage report from all swift_io tests
...
echo "Generating swift_io coverage report..."
echo "========================================"
...
find twister-combined-coverage -name "*.gcda" -o -name "*.gcno" | while read file; do
    ...
    testname=$(echo "$file" | sed 's|.*/\(swift_io\.[^/]*\)/.*|\1|')
    if [[ "$file" == *.gcda ]]; then
...
gcovr --filter=".*swift_io/subsys/swift_io/.*" --print-summary \

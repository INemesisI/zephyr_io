...
NET_BUF_POOL_DEFINE(runtime_pool, TEST_BUFFER_POOL_SIZE, TEST_BUFFER_SIZE, 4,
                    NULL);
...
static void runtime_immediate_handler(struct flow_sink *sink,
                                      struct net_buf *buf) {
...
static void runtime_queued_handler(struct flow_sink *sink,
                                   struct net_buf *buf) {
  struct test_capture *capture = (struct test_capture *)sink->user_data;
...
FLOW_SINK_DEFINE_QUEUED(runtime_queued_sink, runtime_queued_handler,
                        runtime_queue);
...
static void init_sinks(void) {
  ...
  runtime_queued_sink.user_data =
      &runtime_capture1; /* Share capture with immediate */
}
...
ZTEST(flow_runtime, test_basic_add_remove) {
  ...
  zassert_equal(runtime_capture1.last_value, TEST_VALUE_1,
...
ZTEST(flow_runtime, test_multiple_connections) {
   ...
   */
  zassert_equal(runtime_capture1.last_value, TEST_VALUE_3,
                "Wrong value in capture1");
  zassert_equal(runtime_capture2.last_value, TEST_VALUE_3,
                "Wrong value in capture2");
  ...
  zassert_equal(atomic_get(&runtime_capture1.count), 2,
                "capture1 should get both");
  zassert_equal(atomic_get(&runtime_capture2.count), 1,
                "capture2 should get one");
  ...
  zassert_equal(runtime_capture1.last_value, TEST_VALUE_1,
                "Wrong value in capture1");
  zassert_equal(runtime_capture2.last_value, TEST_VALUE_1,
                "Wrong value in capture2");
...
ZTEST(flow_runtime, test_connection_persistence) {
  ...
  zassert_equal(runtime_capture2.last_value, TEST_VALUE_2,
                "Wrong value in capture2");
...
ZTEST(flow_runtime, test_cross_source_connections) {
  ...
  zassert_equal(atomic_get(&runtime_capture1.count), 1,
                "capture1 should receive once");
  zassert_equal(atomic_get(&runtime_capture2.count), 1,
                "capture2 should receive once");
  ...
  zassert_equal(atomic_get(&runtime_capture2.count), 0,
                "capture2 should not receive");

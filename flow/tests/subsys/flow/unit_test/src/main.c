...
{
	int count = 0;
	while (flow_event_process(&test_events, K_NO_WAIT) == 0 && count < TEST_MAX_EVENTS) {
		count++;
	}
}
...
ZTEST(flow_unit_test, test_invalid_sink_mode)
{
	...
	struct flow_sink test_sink = FLOW_SINK_INITIALIZER(
		test_sink, SINK_MODE_IMMEDIATE, immediate_handler, NULL, NULL, FLOW_PACKET_ID_ANY);
...
ZTEST(flow_unit_test, test_chaining_double_unref_bug)
{
	...
	struct flow_source test_source = FLOW_SOURCE_INITIALIZER(test_source, FLOW_PACKET_ID_ANY);
	struct flow_sink chaining_sink =
		FLOW_SINK_INITIALIZER(chaining_sink, SINK_MODE_IMMEDIATE, chaining_handler, NULL,
...
FLOW_SINK_DEFINE_ROUTED_IMMEDIATE(routed_sink1, immediate_handler, TEST_PACKET_ID_1);
FLOW_SINK_DEFINE_ROUTED_IMMEDIATE(routed_sink2, immediate_handler, TEST_PACKET_ID_2);
FLOW_SINK_DEFINE_IMMEDIATE(accept_all_sink, immediate_handler); /* Accepts any ID */

/* Define routed queued sinks */
FLOW_SINK_DEFINE_ROUTED_QUEUED(routed_queued_sink1, capture_handler, test_events, TEST_PACKET_ID_1);
FLOW_SINK_DEFINE_ROUTED_QUEUED(routed_queued_sink2, capture_handler, test_events, TEST_PACKET_ID_2);
...
static void routed_immediate_handler1(struct flow_sink *sink, struct net_buf *buf)
{
	atomic_inc(&routed_immediate_count1);
}

static void routed_immediate_handler2(struct flow_sink *sink, struct net_buf *buf)
{
...
ZTEST(flow_unit_test, test_packet_id_stamping)
{
	...
	*pkt_id = 0x0000;
	...
	zassert_equal(*pkt_id, TEST_PACKET_ID_1,
		      "Packet ID should be stamped as 0x%04x, got 0x%04x", TEST_PACKET_ID_1,
		      *pkt_id);
...
ZTEST(flow_unit_test, test_packet_id_filtering)
{
	 ...
	 */
	struct flow_connection conn1 = {.source = &broadcast_source, .sink = &routed_sink1};
	struct flow_connection conn2 = {.source = &broadcast_source, .sink = &routed_sink2};
	struct flow_connection conn3 = {.source = &broadcast_source, .sink = &accept_all_sink};
	...
	*pkt_id = TEST_PACKET_ID_1;
	...
	zassert_equal(atomic_get(&routed_immediate_count1), 1, "routed_sink1 should receive");
	zassert_equal(atomic_get(&routed_immediate_count2), 0, "routed_sink2 should not receive");
	...
	*pkt_id = TEST_PACKET_ID_2;
	...
	zassert_equal(ret, 2, "Should deliver to 2 sinks (matching + accept_all)");
	zassert_equal(atomic_get(&routed_immediate_count1), 0, "routed_sink1 should not receive");
	zassert_equal(atomic_get(&routed_immediate_count2), 1, "routed_sink2 should receive");
	zassert_equal(atomic_get(&accept_all_count), 1, "accept_all should receive");
...
ZTEST(flow_unit_test, test_packet_id_any_broadcast)
{
	...
	*pkt_id = FLOW_PACKET_ID_ANY;
	...
	zassert_equal(atomic_get(&accept_all_count), 1, "accept_all should accept broadcast");
...
ZTEST(flow_unit_test, test_routed_source_to_routed_sink)
{
	 ...
	 */
	struct flow_connection conn1 = {.source = &routed_source1, .sink = &routed_sink1};
	struct flow_connection conn2 = {.source = &routed_source1, .sink = &routed_sink2};
	struct flow_connection conn3 = {.source = &routed_source2, .sink = &routed_sink1};
	struct flow_connection conn4 = {.source = &routed_source2, .sink = &routed_sink2};
...
ZTEST(flow_unit_test, test_routed_queued_sinks)
{
	...
	struct flow_connection conn1 = {.source = &routed_source1, .sink = &routed_queued_sink1};
	struct flow_connection conn2 = {.source = &routed_source1, .sink = &routed_queued_sink2};
	struct flow_connection conn3 = {.source = &routed_source2, .sink = &routed_queued_sink2};
...
ZTEST(flow_unit_test, test_mixed_routed_topology)
{
	 ...
	 */
	struct flow_connection conn1 = {.source = &routed_source1, .sink = &routed_sink1};
	struct flow_connection conn2 = {.source = &routed_source1, .sink = &accept_all_sink};
	struct flow_connection conn3 = {.source = &broadcast_source, .sink = &routed_sink1};
	struct flow_connection conn4 = {.source = &broadcast_source, .sink = &routed_sink2};
	struct flow_connection conn5 = {.source = &broadcast_source, .sink = &accept_all_sink};
	...
	zassert_equal(ret, 2, "Should deliver to matching routed_sink1 and accept_all");
	zassert_equal(atomic_get(&routed_immediate_count1), 1, "routed_sink1 should receive");
	zassert_equal(atomic_get(&routed_immediate_count2), 0, "routed_sink2 should not receive");
	zassert_equal(atomic_get(&accept_all_count), 1, "accept_all should receive");
...
ZTEST(flow_unit_test, test_packet_id_overwrite)
{
	...
	struct flow_connection conn = {.source = &routed_source1, .sink = &accept_all_sink};
...
ZTEST(flow_unit_test, test_packet_id_stats)
{
	...
	zassert_equal(queued_total, 5, "Should have delivered 5 (only to matching sink)");
	...
	zassert_equal(handled_count, 0, "routed_sink2 should handle 0 packets (filtered)");
	zassert_equal(dropped_count, 0, "routed_sink2 should have no drops (not attempted)");

...
FLOW_SINK_DEFINE_QUEUED(processor_outbound_sink, outbound_handler,
                        processor_queue);

/* Define inbound sink for packets from echo server */
FLOW_SINK_DEFINE_QUEUED(processor_inbound_sink, inbound_handler,
                        processor_queue);
...
static void outbound_handler(struct flow_sink *sink, struct net_buf *buf) {
  ...
  LOG_INF("Processed: Sensor %d, %zu bytes (header %zu + payload %u)",
          packet_id, total_len, sizeof(*header), header->content_length);
...
K_THREAD_DEFINE(processor_thread, 1024, processor_thread_fn, NULL, NULL, NULL,
                7, 0, 0);
...
static void processor_handler(struct flow_sink *sink, struct net_buf *buf);

/* Define processor sink with queued handler and source */
FLOW_SINK_DEFINE_QUEUED(processor_sink, processor_handler, processor_queue);
FLOW_SOURCE_DEFINE(processor_source);
...
static void processor_handler(struct flow_sink *sink, struct net_buf *buf)
{
	...
	flow_source_send(&processor_source, header_buf, K_NO_WAIT);
}
